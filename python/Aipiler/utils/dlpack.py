...
from typing import List, Callable, Set, Tuple, Iterable
import ctypes
...
class DLDataType(ctypes.Structure):
    ...
    def to_dtype(dl_dtype: DLDataType) -> DataType:
        ...
        if dtype_lanes > 1:
            raise ValueError(
                "from_dlpack: only tensors with lanes=1 are supported for aipiler"
            )
        if dtype_code == DLDataTypeCode.kDLInt:
            ...
            raise ValueError("from_dlpack: opaque handle is not supported for aipiler")
        ...
        if dtype_name != "bool":
            ...
            if dtype_bits != 8:
                raise ValueError(
                    "from_dlpack: aipiler only supports bool dtype with 8 bits."
                )
            dtype = dtype_name
        if not Aipiler.datatype.supported(dtype):
            raise ValueError(
                "from_dlpack: dtype {} is not supported for aipiler".format(dtype)
            )
...
class DLManagedTensor(ctypes.Structure):
    _fields_ = [
        ("dl_tensor", DLTensor),
        ("manager_ctx", ctypes.c_uint64),
        ("deleter", DLTensorDeleter),
    ]
...
class DLPackStorage(Storage):
    def __init__(
        ...
        addr: int,
        num_bytes: int,
        managed_tensor_addr: int,
        deleter: Callable[[int], None],
    ):
        super().__init__(
            device=device,
            addr=addr,
            num_bytes=num_bytes,
            free_handler=lambda storage: deleter(managed_tensor_addr),
        )
...
def from_dlpack_capsule(dltensor) -> Tensor:
    if pythonapi.PyCapsule_IsValid(dltensor, b"dltensor"):
        ...
        managed_tensor: DLManagedTensor = ctypes.cast(
            ptr, ctypes.POINTER(DLManagedTensor)
        ).contents
        ...
        if device_type in [DLDeviceType.kDLCPU, DLDeviceType.kDLCPUPinned]:
            ...
            raise ValueError(
                "from_dlpack: currently, aipiler only supports only CPU and GPU tensors."
            )
            ...
            if not is_compact_tensor(shape, strides):
                raise ValueError(
                    (
                        ...
                        "aipiler, please consider make it continuous before passing it to aipiler."
...
class DLManagedTensorContext:
    ...
    def __init__(self, tensor: Tensor):
        ...
        self.managed_tensor = DLManagedTensor(
            dl_tensor=dl_tensor, manager_ctx=0, deleter=DLTensorDeleter(deleter)
        )

    def capsuled_dltensor(self) -> ctypes.py_object:
        return pythonapi.PyCapsule_New(
            ctypes.byref(self.managed_tensor), b"dltensor", None
        )
