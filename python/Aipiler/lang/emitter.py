...
from iree.compiler.dialects import func, linalg, math, arith, complex
from iree.compiler.dialects._ods_common import (
...
class _BodyBuilder:
    ...
    # 新加函数
    def _binary_div(self, lhs: Value, rhs: Value) -> Value:
        if _is_floating_point_type(lhs.type):
            return arith.DivFOp(lhs, rhs).result
        if _is_interger_sign_type(lhs.type):
            return arith.DivSIOp(lhs, rhs).result
        raise NotImplementedError("Unsupported 'mul' operands: {lhs}, {rhs}")

    # 新加函数
    def _binary_div_unsigned(self, lhs: Value, rhs: Value) -> Value:
        if _is_interger_unsign_type(lhs.type) or _is_index_type(lhs.type):
            return arith.DivUIOp(lhs, rhs).result
        raise NotImplementedError("Unsupported 'mul' operands: {lhs}, {rhs}")
...
# 新加函数
def _is_interger_sign_type(t: Type) -> bool:
    return IntegerType.is_signed(t)


# 新加函数
def _is_interger_unsign_type(t: Type) -> bool:
    return IntegerType.is_unsigned(t)
