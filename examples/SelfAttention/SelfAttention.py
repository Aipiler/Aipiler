...
hidden_states = torch.randn([1, 5, 5120], dtype=torch.float32)
residual =  torch.randn([1, 5, 5120], dtype=torch.float32)
attention_mask = torch.ones([1, 1, 5, 5], dtype=torch.bool)
# mlir_module = torchscript.compile(model, [hidden_states, residual, attention_mask])
ret = model(hidden_states, residual, attention_mask)
print(ret)