...
void generateCode(mlir::ModuleOp &theModule, mlir::OpBuilder &builder) {
  ...
  // Graph0
  auto tensorType = RankedTensorType::get({4, 3}, elementType);
  ...
  auto argAttr = DenseElementsAttr::get(
      tensorType, llvm::ArrayRef<float>{1, 2, 3, 3, 2, 1, 4, 5, 6, 6, 5, 4});

  auto arg0 = builder.create<arith::ConstantOp>(loc, argAttr);
  auto gelu = builder.create<func::CallOp>(loc, graph0, ValueRange{arg0});
  auto cast =
...
// 	reconcile-unrealized-casts)"
  ...
  generateExecutable(*llvmModule.get(), "gelu");
